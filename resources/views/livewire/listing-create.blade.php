<div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow">
    ...
    <form wire:submit.prevent="submit">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                <label>Şehir</label>
                <select wire:model.live="selectedCity" class="w-full border rounded px-3 py-2">
                    <option value="">Seçiniz</option>
                    @foreach ($cities as $c)
            ...
            <div>
                <label>İlçe</label>
                <select wire:model.live="selectedDistrict" class="w-full border rounded px-3 py-2"
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div>
                <label>Mahalle</label>
                <select wire:model.live="selectedNeigborhood" class="w-full border rounded px-3 py-2"
            ...
            <div>
                <label>Quarter</label>
                <select wire:model.live="selectedQuarter" class="w-full border rounded px-3 py-2"
        ...
        <div class="mb-4">
            ...
            <input type="file" multiple wire:model.defer="photos" class="w-full border rounded px-3 py-2">
            @error('photos.*')
                <span class="text-red-600 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        {{-- Gönder --}}
        <button type="submit" class="w-full py-2 px-4 bg-green-600 text-white font-semibold rounded">
            İlanı Yayınla
        </button>
    </form>
</div>
